...
def _fit(X, max_iter=500, mc_iter=1, persistent=True):
    """CD k(==1) algo.
...
W = _fit(X, max_iter=500, mc_iter=1, persistent=False)

# x = np.random.randint(2, size=(n_features,))
x = proposed(X[3], p=0.12)
xx = random_walk(x, energy=lambda _: _energy(_, W), proposed=proposed, mc_iter=200)

import matplotlib.pyplot as plt
fig = plt.figure()
ax = fig.subplots(1, 2)
ax[0].imshow(np.delete(X[3], -1).reshape((8, 8)))
ax[0].set_title('A real digit')
ax[1].imshow(np.delete(xx, -1).reshape((8, 8)))
ax[1].set_title('A generated digit')
